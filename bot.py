...
async def handle_important_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "2. Оформляет предложение по форме и отправляет в телеграмм бот (Предложения по улучшениям Мечел-Материалы)
        или передает своему руководителю в подразделении. (руководитель должен выдать бланк) \n"
